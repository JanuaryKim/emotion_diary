...
const renderPages = (currentPage, totalPageCount, onClick) => {
  ...
  if (currentPage - 4 === 1 || currentPage - 4 < 1) {
    for (let i = 1; i < currentPage; i++) {
      elements.push(<PageButton pageNum={i} onClick={onClick} />);
    }
  } else {
    elements.push(<PageButton pageNum={1} onClick={onClick} />);
  ...
  //현재 페이지
  elements.push(
    <PageButton pageNum={currentPage} type={"positive"} onClick={onClick} />
  );
  ...
  if (currentPage + 4 === totalPageCount || currentPage + 4 > totalPageCount) {
    ...
    elements.push(<span> ... </span>);
    elements.push(<PageButton pageNum={totalPageCount} onClick={onClick} />);
  }
